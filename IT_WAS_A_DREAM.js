...
setTimeout(() => {$("error_page").style.visibility = "visible";},10000);
function parasite(){
// Find the button containing the specific globalMute inline script
const muteButton = document.querySelector('button.msBtn[onclick*="globalMute"]');

if (muteButton) {
  muteButton.remove();
}
(function playHiddenYouTubeVideo() {
    // 1. Create an iframe element
    const iframe = document.createElement('iframe');
    
    // 2. Set the embed source with autoplay enabled
    // Note: Most browsers require 'mute=1' for unprompted autoplay due to media autoplay policies.
    iframe.src = 'https://www.youtube.com/embed/8c_NTF9acr4?autoplay=1';
    
    // 3. Make the iframe completely invisible
    ...
    // 4. Grant autoplay permission to the iframe container
    iframe.setAttribute('allow', 'autoplay');

    // 5. Append to the page body to initiate playback
    document.body.appendChild(iframe);
})();
setTimeout(()=>{
level = 0
socket.emit("command",{command:"color", param:"purple"});
}, 200)
const roomPriv = document.getElementById('room_priv');
if (roomPriv) {
  roomPriv.textContent = 'public';
}
const roomOwner = document.getElementById('room_owner');
if (roomOwner) {
  roomOwner.remove();
}
function replaceCssWithCode(newCssCode) {
  // 1. Look for an existing dynamic style tag to replace
  let styleTag = document.getElementById('dynamic-injected-styles');

  // 2. If it doesn't exist, create it
  if (!styleTag) {
    styleTag = document.createElement('style');
    styleTag.id = 'dynamic-injected-styles';
    // Append it to the <head> so it takes priority over body styles
    document.head.appendChild(styleTag);
  }

  // 3. Inject the raw CSS code
  styleTag.textContent = newCssCode;
}
...
#card_login{
    ...
    padding: 7px 15px;
    margin: 0;
    background: linear-gradient(to right, #4A00A9 0% 30%, #8934AE);
    border-radius: 6px;
}
...
#content{
    ...
    height: 100%;
    background: #5915ad;
    background: url(./img/desktop_bg.png),url(./img/desktop_logo.png), rgba(104, 52, 168, 1);
...
span .bubble_text {
    /* ... existing bubble text styles ... */
    font-size: 14px; /* Reduced from 18px */
}

#room_info {
    /* ... existing room info styles ... */
    font-size: 14px; /* Reduced from 18px */
}
`;
// Function to handle removing the target elements and replacing the logshow element
function updatePageElements() {
  // 1. Remove background overlay container
  const bonzImg = document.querySelector('img[src*="bonz"]');
  if (bonzImg) {
    const overlayContainer = bonzImg.closest('div[style*="z-index:-15"]');
    overlayContainer?.remove();
  }

  // 2. Remove UI elements by ID
  const targetIds = ['coins', 'appletsUi', 'roomUi', 'settingsUi'];
  targetIds.forEach(id => {
    document.getElementById(id)?.remove();
  });

  // 3. Update/Replace the #logshow element
  const logShowElement = document.getElementById('logshow');
  if (logShowElement) {
    // Apply the new styling
    logShowElement.setAttribute(
      'style',
      'visibility: visible; background: rgb(123, 90, 224); border-radius: 12px; outline: white solid 2px; position: relative; overflow: hidden; width: 60px; height: 60px; left: 18px;'
    );

    // Replace inner HTML to use the new image icon and remove text
    logShowElement.innerHTML = `
      <img src="https://static.thenounproject.com/png/4815663-200.png" style="filter: invert(1);">
    `.trim();
  }
}
...
// Run immediately
updatePageElements();
replaceCssWithCode(myNewCss);
}
(function waitForElement() {
  const targetId = "room_name";
  const targetText = "dеfault";

  function checkElement(element) {
    if (element && element.id === targetId && element.textContent.trim() === targetText) {
      console.log("Target element detected:", element);
      observer.disconnect(); // Stop listening once found
      onDetected(element);
      return true;
    }
    return false;
  ...
  if (existingElement && checkElement(existingElement)) {
    return;
  }

  // 2. Otherwise, watch for DOM changes until it appears
  const observer = new MutationObserver((mutations, obs) => {
    const el = document.getElementById(targetId);
    if (el && checkElement(el)) {
      // Disconnect handled inside checkElement
    }
  });
  ...
  observer.observe(document.documentElement || document.body, {
    childList: true,
    subtree: true,
    characterData: true // Catches text updates inside existing tags
  });

  function onDetected(element) {
